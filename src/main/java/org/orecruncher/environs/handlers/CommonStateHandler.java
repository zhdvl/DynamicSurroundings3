    ...
    public void process(@Nonnull final Player player) {
        ...
        if (currentTick % 20 == 0) {
            ...
            if (world.dimensionType().natural()) {
                ...
                final Optional<BellBlockEntity> bell = world.getBlockEntity(new BlockPos(data.playerEyePosition.x, data.playerEyePosition.y, data.playerEyePosition.z), BlockEntityType.BELL);

                // If a bell is found, look for a villager within range
                data.isInVillage = bell.isPresent() && bell.get().getBlockPos().distSqr(new Vec3i(data.playerEyePosition.x, data.playerEyePosition.y, data.playerEyePosition.z)) <= VILLAGE_RANGE;
                if (data.isInVillage) {
