...
public final class BlockStateLibrary {
    ...
    private static Collection<BlockStateMatcher> expand(@Nonnull final String blockName) {
        if (blockName.startsWith(TAG_SPECIFIER)) {
            ...
            final Registry<Block> tag = TagUtils.getBlockTag(tagName);
            if (tag != null) {
                return tag.stream().filter(block -> block.asItem().getRegistryName().toString().equals(tagName)).map(BlockStateMatcher::create).filter(m -> !m.isEmpty()).collect(Collectors.toList());
            }
