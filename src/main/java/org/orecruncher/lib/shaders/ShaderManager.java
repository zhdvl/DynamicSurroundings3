...
//		}
	...
	static GlslPreprocessor preprocessor = new GlslPreprocessor() {
		@Override
		public @NotNull String applyImport(boolean p_166480_, @NotNull String p_166481_) {
			return p_166481_;
		}
	};
	private static Program createShader(@Nonnull final ResourceManager manager, @Nonnull final ResourceLocation loc, @Nonnull final Program.Type shaderType) throws IOException {
		try (InputStream is = new BufferedInputStream(manager.getResource(loc).getInputStream())) {
			return Program.compileShader(shaderType, loc.toString(), is, shaderType.name().toLowerCase(Locale.ROOT), preprocessor);
		}
	}
}
